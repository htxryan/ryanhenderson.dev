...
ca loop --epics "id1,id2,id3" \
  --model "Codex-opus-4-7[1m]" \
  --reviewers "Codex-sonnet,Codex-opus,gemini,codex" \
  --review-every 1 \
  --max-review-cycles 3 \
  --max-retries 1 \
  --force
...
| `--model` | `Codex-opus-4-7[1m]` | Model for implementation sessions |
| `--reviewers` | (none) | Comma-separated: `Codex-sonnet,Codex-opus,gemini,codex` |
...
| `--review-model` | `Codex-opus-4-7[1m]` | Model for implementer fix sessions |
...
| `--model` | `Codex-opus-4-7[1m]` | Model for polish architect sessions |
| `--reviewers` | `Codex-sonnet,Codex-opus,gemini,codex` | Comma-separated audit fleet |
...
4. Verify `Codex` CLI is available and authenticated
...
|-----|---------------------|------------|
| `Codex` | `-p "prompt"` | `--model <id>` |
| `gemini` | `-p "prompt"` | `-m <model>` |
| `codex` | `codex exec "prompt"` | (default model) |

Stdin piping works for all three: `cat file.md | Codex -p "Review this"`.
...
- Do not use `gemini --print`, `codex --print`, or `Codex --print` — wrong flags
- Do not use `Codex -m sonnet` — use `Codex --model Codex-sonnet-4-6`
