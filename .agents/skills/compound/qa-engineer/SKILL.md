...
3. **Capture** across all channels:
   - **Visual**: Full-page screenshot (Codex can read this as a multimodal image)
...
Systematically probe for failures. Read `.Codex/skills/compound/qa-engineer/references/exploratory-testing.md` for the full playbook. In brief:
...
Use Codex's multimodal vision to evaluate the UI:
...
For repeated QA of the same pages, you can define test constitutions -- persistent JSON files that describe what to test. Read `.Codex/skills/compound/qa-engineer/references/constitution-schema.md` for the format. Constitutions are optional -- the skill works without them by using auto-discovery from the reconnaissance phase.

## Browser Automation Patterns

For detailed Playwright patterns (selectors, screenshots, console capture, network interception, viewport emulation), read `.Codex/skills/compound/qa-engineer/references/browser-automation.md`.
...
The runtime-verifier agent (`.Codex/skills/compound/agents/runtime-verifier/SKILL.md`) also tests running applications, but serves a different purpose:
